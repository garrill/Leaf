...
- **Column layout is native**: a 3-column `NavigationSplitView` (sidebar | branch/history | detail, Mail's shape) with an `HSplitView` inside `detail` for files | diff — NavigationSplitView tops out at three columns. Resizing is all native dividers (the old hand-rolled `HStack` + `ResizableDivider` approach is gone). Every column keeps its `List` permanently mounted, swapping empty/error states via `ZStack`/`.opacity()`.
- **Per-column toolbar sections are not achievable in SwiftUI** (tried extensively 2026-07): toolbar items can't be pinned over a given column — `.navigation` lands at leading, `.principal` centers in the window (a flexible `ToolbarSpacer` drags `.primaryAction` items with it), and items declared on the content column's own `.toolbar` still merge into the window-level trailing group. Width-tracked custom toolbar items (fixed `.frame` + `.id` to force NSToolbar re-measure) don't reliably follow column resizes either. The Mail-style column-aligned toolbar would need AppKit `NSSplitViewController` + `NSTrackingSeparatorToolbarItem`. Also: views layered under the titlebar via `.ignoresSafeArea(.top)` render fine but can never receive clicks (the titlebar/toolbar view swallows them), and hiding the window toolbar with `.toolbarVisibility(.hidden, for: .windowToolbar)` removes the traffic lights too. The soft scroll edge blur only renders against a column's own `safeAreaBar` — content merely scrolling under the toolbar gets no effect.
